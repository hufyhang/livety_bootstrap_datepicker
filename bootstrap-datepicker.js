  ...
  var __isMobile = document.getElementsByTagName('html')[0].className.indexOf('touch') !== -1
	...
	var Datepicker = function(element, options){
        ...
        if (this.isInput){
            if(__isMobile){
                ...
                touchendEvent = function(elem){
                    return function(e){
                        elem.focus();
                        return false;
                    }
                }
                touchmoveEvent =function(elem){
                    elem.data('isBind', false);
                    elem.data('initial', true);

                    return function(){
                        elem.isBind = elem.data('isBind');
                        if(!elem.isBind){
                            elem.data('isBind', true);
                            elem.isBind=elem.data('isBind');
                            elem.off('touchend.datepicker');
                            elem.off('touchstart.datepicker');
                            elem.one('touchstart.datepicker', function(e){
                                elem.data('isBind', false);
                                elem.isBind=elem.data('isBind');
                                elem.on('touchend.datepicker', touchendEvent(elem));
                            })
                        }
                    }
                }
                if(!this.element.data('initial')){
                    this.element.on('touchend.datepicker', touchendEvent(this.element));
                    $(document).on('touchmove.datepicker', touchmoveEvent(this.element));
                }
            }
        }
	};
    ...
    handleMinViewMode: function (formatted, minViewMode) {
      ...
      if (minViewMode === 1) {
        ...
        var _tokens = _format.split(_combinator);
        var _dayIndex = _tokens.indexOf('d');
        var _monthIndex = _tokens.indexOf('m');
        var _yearIndex = _tokens.indexOf('Y');

        var items = formatted.split(_combinator);
        items.splice(_dayIndex, 1);
        formatted = items.join(_combinator);
      }
      return formatted;
    },


		setValue: function(minViewMode){
      ...
      this.updateNavArrows();
		...
		place: function(){
			...
			var calendarWidth = this.picker.outerWidth(),
                ...
                currentInputOffsetTop = this.element.offset().top -scrollTop,//input相对于当前视口的顶部的距离
                heightDiff = currentInputOffsetTop - windowHeight/2;
                //if(windowWidth<768)
                if(__isMobile) {
                  $("body, html").scrollTop(scrollTop+heightDiff);
                }
           ...
           var availWeight = parseInt(window.innerWidth);
            //如果适口大小小于等于768px by gyy
            var datepickerBackground= $(".datepicker-background");
			datepickerBackground.on('touchend',function(e){
				e.preventDefault();
			})
            //if(availWeight <=768){
            if(__isMobile) {
                if(this.picker.css('display') === 'block' && !datepickerBackground.hasClass("show")){
                    datepickerBackground.addClass("show");
                    $("body").css({"position":"relative","overflow":"hidden"});
                }
                else{
                    if(datepickerBackground.hasClass("show") && !(this.picker)[0].style){
                        datepickerBackground.removeClass("show");
                ...
                var windowWidth = $window.width(),
                    ...
                    scrollTop = $window.scrollTop();
                this.picker.css({
                        top: (windowHeight/2 - calendarHeight/2)+scrollTop,
                        //left: (windowWidth/2 - calendarWidth/2)+"px",
                        zIndex: 9
                    }
                )
            }else{
                 //by gyy
                if(datepickerBackground.hasClass("show")){
                    datepickerBackground.removeClass("show");
